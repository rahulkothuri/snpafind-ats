...
router.get('/candidates', authenticate, async (req, res, next) => {
    try {
        ...
        const searchResults = await searchService.searchCandidates(req.user.companyId, searchQuery);
...
router.get('/jobs', authenticate, async (req, res, next) => {
    try {
        ...
        const searchResults = await searchService.searchJobs(req.user.companyId, searchQuery);
        // Parse Boolean query for debugging/validation
        const parsedQuery = searchService.parseBooleanQuery(searchParams.query);
        res.json({
            ...searchResults,
...
router.get('/suggestions', authenticate, async (req, res, next) => {
    try {
        ...
        const suggestions = await getSearchSuggestions(req.user.companyId, query, type);
...
router.get('/filters', authenticate, async (req, res, next) => {
    try {
        ...
        const filterOptions = await getFilterOptions(req.user.companyId, type);
 ...
 */
async function getSearchSuggestions(companyId, query, type) {
    const suggestions = [];
 ...
 */
async function getFilterOptions(companyId, type) {
    const { prisma } = await import('../lib/prisma.js');
    if (type === 'candidates') {
