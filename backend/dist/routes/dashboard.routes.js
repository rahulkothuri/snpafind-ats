...
router.get('/pipeline', authenticate, async (req, res, next) => {
    try {
        ...
        const rolePipeline = await getRolePipelineData(req.user.companyId, req.user.userId, req.user.role, parseInt(limit), sortBy, sortOrder);
 ...
 */
router.get('/interviews', authenticate, async (req, res, next) => {
    try {
        ...
        const interviews = await getUpcomingInterviews(req.user.companyId, req.user.userId, period, parseInt(limit));
        res.json(interviews);
    }
    catch (error) {
        next(error);
...
router.get('/activity', authenticate, async (req, res, next) => {
    try {
        ...
        const activityFeed = await getActivityFeed(req.user.companyId, parseInt(limit), type);
 ...
 */
async function getRolePipelineData(companyId, userId, userRole, limit = 7, sortBy = 'priority', sortOrder = 'desc') {
 ...
 * Get upcoming interviews with period filtering
 */
async function getUpcomingInterviews(companyId, userId, period, limit = 5) {
    const now = new Date();
 ...
 */
async function getActivityFeed(companyId, limit = 10, type) {
