 ...
 */
router.get('/from-jobs', async (req, res, next) => {
    try {
        const { userId, role, companyId } = req.user;
 ...
 */
router.post('/import-from-job', async (req, res, next) => {
    try {
        const { userId, role, companyId } = req.user;
        ...
        const template = await stageTemplateService.importFromJob(jobId, templateName, templateDescription || '', userId, role, companyId);
        res.status(201).json({
            success: true,
            data: template,
        });
...
//# sourceMappingURL=stageTemplates.js.map