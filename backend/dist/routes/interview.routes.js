...
router.get('/', authenticate, async (req, res, next) => {
    try {
        ...
        const filters = {
            ...
            panelMemberId: req.query.panelMemberId,
            status: req.query.status,
            mode: req.query.mode,
            jobCandidateId: req.query.jobCandidateId,
        };
        // Parse date filters
 ...
 */
router.get('/panel-load', authenticate, async (req, res, next) => {
    try {
        const companyId = req.user.companyId;
 ...
 * Requirements: 14.2
 */
router.get('/pending-feedback', authenticate, async (req, res, next) => {
    try {
        const userId = req.user.userId;
...
router.post('/:id/feedback', authenticate, async (req, res, next) => {
    try {
        ...
        feedbackService.processAutoStageMovement(id).catch((err) => {
 ...
 * Requirements: 14.5
 */
router.get('/:id/feedback', authenticate, async (req, res, next) => {
    try {
        const { id } = req.params;
 ...
 * Requirements: 14.5
 */
router.get('/:id/feedback-status', authenticate, async (req, res, next) => {
    try {
        const { id } = req.params;
