...
router.get('/', authenticate, async (req, res, next) => {
    try {
        ...
        const tasks = await taskService.getByUserId(req.user.userId, req.user.companyId, status);
        res.json(tasks);
    }
    catch (error) {
        next(error);
...
router.post('/', authenticate, async (req, res, next) => {
    try {
        ...
        const task = await taskService.create(req.user.userId, req.user.companyId, result.data);
...
router.put('/:id', authenticate, async (req, res, next) => {
    try {
        ...
        const task = await taskService.update(req.params.id, req.user.userId, result.data);
        res.json(task);
    }
    catch (error) {
        next(error);
