...
export const pipelineService = {
    ...
    async getStagesByJobId(jobId) {
        ...
        return stages.map((s) => this.mapToStage(s));
    },
    /**
     * Insert a custom sub-stage at a specific position
     * Requirements: 6.2
     */
    async insertStage(data) {
        // Validate input
    ...
    async reorderStage(data) {
        ...
        if (oldPosition === newPosition) {
            return allStages.map((s) => this.mapToStage(s));
        }
        ...
        return updatedStages.map((s) => this.mapToStage(s));
    },
    /**
    ...
    async addSubStage(data) {
        ...
        const existingSubStage = parentStage.subStages.find((s) => s.name.toLowerCase() === data.name.trim().toLowerCase());
        if (existingSubStage) {
            throw new ValidationError({
        ...
        if (data.position !== undefined) {
            ...
            const maxPosition = parentStage.subStages.reduce((max, s) => Math.max(max, s.position), -1);
