...
export const notificationService = {
    ...
    async getPendingFeedbackInterviews(companyId) {
    ...
    async processFeedbackPendingNotifications(hoursThreshold = 24) {
        const result = { notificationsCreated: 0, emailsSent: 0, errors: [] };
                        ...
                        try {
                            ...
                            if (!existingNotification) {
                                ...
                                if (emailResult.success)
                                    ...
                                    result.errors.push(`Failed to send email to ${pm.user.email}: ${emailResult.error}`);
                            }
                        }
                        catch (error) {
    ...
    async checkAndMarkFeedbackComplete(interviewId) {
        const interview = await prisma.interview.findUnique({
            where: { id: interviewId },
