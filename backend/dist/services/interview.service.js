...
export const interviewService = {
    ...
    async updateInterview(id, data) {
        ...
        try {
            await interviewActivityService.recordInterviewRescheduled(mappedInterview, oldScheduledAt, oldTimezone, oldDuration, oldMode, oldLocation ?? undefined);
        }
        catch (error) {
     ...
     */
    async getDashboardInterviews(userId, companyId) {
        const now = new Date();
     ...
     */
    async getPanelLoadDistribution(companyId, period) {
        const now = new Date();
        ...
        // Convert to array
        const panelLoads = Array.from(panelLoadMap.values()).map((p) => ({
            userId: p.user.id,
