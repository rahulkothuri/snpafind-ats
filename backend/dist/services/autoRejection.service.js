 ...
 */
function convertLegacyRules(legacy) {
    const rules = [];
    let ruleId = 1;
 ...
 */
function evaluateNumericOperator(candidateValue, operator, ruleValue) {
 ...
 */
function evaluateTextOperator(candidateValue, operator, ruleValue) {
    if (candidateValue === undefined || candidateValue === null)
        return false;
 ...
 */
function evaluateArrayOperator(candidateValue, operator, ruleValue) {
    if (!candidateValue || !Array.isArray(candidateValue))
        return false;
 ...
 */
function getCandidateFieldValue(candidate, field) {
    switch (field) {
        case 'experience':
 ...
 */
function evaluateSingleRule(candidate, rule) {
    const candidateValue = getCandidateFieldValue(candidate, rule.field);
    // Determine field type and evaluate accordingly
    switch (rule.field) {
        case 'experience':
        case 'salary_expectation':
            return evaluateNumericOperator(candidateValue, rule.operator, rule.value);
        case 'location':
        case 'education':
            return evaluateTextOperator(candidateValue, rule.operator, rule.value);
        case 'skills':
            return evaluateArrayOperator(candidateValue, rule.operator, rule.value);
        default:
            return false;
    }
}
 ...
 */
function generateRejectionReason(candidate, rule) {
 ...
 */
export function evaluateAutoRejection(candidate, rules) {
 ...
 */
export async function processAutoRejection(jobCandidateId, candidateId, candidateData, jobId, tx) {
 ...
 */
export async function processAutoRejectionLegacy(jobCandidateId, candidateId, candidateExperience, jobId, tx) {
    return processAutoRejection(jobCandidateId, candidateId, { experience: candidateExperience }, jobId, tx);
}
