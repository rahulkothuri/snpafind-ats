...
export const slaService = {
     ...
     */
    async updateSystemDefaults(configs) {
        // Clear existing system defaults
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await prisma.systemSLADefault.deleteMany({});
        // Insert new defaults
        for (const config of configs) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            await prisma.systemSLADefault.create({
                data: {
     ...
     */
    async getStoredSystemDefaults() {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const storedDefaults = await prisma.systemSLADefault.findMany({
            orderBy: { stageName: 'asc' },
        });
        if (storedDefaults.length > 0) {
            return storedDefaults.map((d) => ({
