...
export const vendorService = {
     ...
     * Get a vendor by ID
     */
    async getVendorById(id, companyId) {
     ...
     * Requirements: 7.7
     */
    async updateVendor(id, companyId, data) {
     ...
     * Requirements: 7.8
     */
    async deactivateVendor(id, companyId) {
     ...
     * Requirements: 10.4
     */
    async assignJobsToVendor(vendorId, companyId, jobIds) {
        // Check if vendor exists
        const vendor = await prisma.user.findFirst({
            where: {
                id: vendorId,
     ...
     */
    async removeJobAssignment(vendorId, companyId, jobId) {
        // Check if vendor exists
        const vendor = await prisma.user.findFirst({
            where: {
                id: vendorId,
     ...
     */
    async getVendorJobIds(vendorId) {
     ...
     */
    async hasJobAccess(vendorId, jobId) {
     ...
     */
    mapToVendor(user) {
        const assignedJobs = user.vendorJobAssignments?.map((vja) => ({
            id: vja.job.id,
            title: vja.job.title,
        })) || [];
        return {
            ...
            assignedJobs,
            vendorJobAssignments: user.vendorJobAssignments?.map((vja) => ({
                id: vja.id,
