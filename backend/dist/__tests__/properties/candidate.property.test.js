...
const nameArbitrary = fc.constantFrom('John Doe', 'Jane Smith', 'Rahul Kumar', 'Priya Sharma', 'Amit Patel', 'Sana Khan', 'Vikram Singh', 'Aarti Gupta');
const emailArbitrary = fc.emailAddress();
const phoneArbitrary = fc.stringMatching(/^\+?[1-9]\d{9,14}$/);
const experienceArbitrary = fc.float({ min: 0, max: 30, noNaN: true })
    .map(n => Math.round(n * 10) / 10);
const locationArbitrary = fc.constantFrom('Bangalore', 'Hyderabad', 'Chennai', 'Pune', 'Remote', 'Gurgaon', 'Mumbai', 'Delhi');
const sourceArbitrary = fc.constantFrom('LinkedIn', 'Referral', 'Job Board', 'Career Page', 'Agency', 'Internal');
const skillsArbitrary = fc.array(fc.constantFrom('Java', 'Spring Boot', 'React', 'Node.js', 'PostgreSQL', 'Kafka', 'Python', 'TypeScript', 'AWS', 'Docker', 'Kubernetes'), { minLength: 0, maxLength: 10 });
const companyArbitrary = fc.constantFrom('FinEdge Systems', 'CloudNova', 'NeoPay', 'CodeNest', 'TechCorp', 'DataFlow', 'InnoSoft');
const ctcArbitrary = fc.integer({ min: 5, max: 100 }).map(n => `${n} LPA`);
const noticePeriodArbitrary = fc.constantFrom('Immediate', '15 days', '30 days', '60 days', '90 days');
const availabilityArbitrary = fc.constantFrom('Immediate', '15 days', '30 days', '60+ days');
...
describe('Property 14: Candidate creation round-trip', () => {
    ...
    it('should return equivalent candidate after create and retrieve', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, nameArbitrary, emailArbitrary, fc.option(phoneArbitrary, { nil: undefined }), experienceArbitrary, fc.option(companyArbitrary, { nil: undefined }), locationArbitrary, sourceArbitrary, skillsArbitrary, async (candidateId, companyId, name, email, phone, experience, currentCompany, location, source, skills) => {
            const now = new Date();
...
describe('Property 15: Candidate IDs are unique', () => {
    ...
    it('should generate unique IDs for all created candidates', async () => {
        await fc.assert(fc.asyncProperty(fc.array(fc.tuple(uuidArbitrary, uuidArbitrary, nameArbitrary, emailArbitrary, locationArbitrary, sourceArbitrary), { minLength: 2, maxLength: 10 }), async (candidateDataList) => {
            const createdIds = [];
            const now = new Date();
...
describe('Property 16: Duplicate email prevention', () => {
    ...
    it('should reject duplicate candidate emails', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, nameArbitrary, emailArbitrary, locationArbitrary, sourceArbitrary, async (existingId, companyId, name, email, location, source) => {
            const now = new Date();
            const normalizedEmail = email.trim().toLowerCase();
    ...
    it('should allow creating candidates with different emails', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, nameArbitrary, emailArbitrary, locationArbitrary, sourceArbitrary, async (candidateId, companyId, name, email, location, source) => {
            const now = new Date();
            const normalizedEmail = email.trim().toLowerCase();
            // Mock findUnique to return null (no duplicate)
            mockPrismaCandidate.findUnique.mockResolvedValueOnce(null);
...
//# sourceMappingURL=candidate.property.test.js.map