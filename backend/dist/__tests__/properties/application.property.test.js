...
import { validatePublicResumeFile, ALLOWED_EXTENSIONS, ALLOWED_MIME_TYPES, MAX_FILE_SIZE } from '../../routes/public.routes.js';
...
const validMimeTypeArbitrary = fc.constantFrom('application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
const invalidMimeTypeArbitrary = fc.constantFrom('text/plain', 'image/jpeg', 'image/png', 'application/zip', 'text/html', 'application/javascript');
...
const validEmailArbitrary = fc.constantFrom('test@example.com', 'applicant@test.org', 'candidate@company.io', 'user@acme.com');
const validNameArbitrary = fc.constantFrom('John Doe', 'Jane Smith', 'Bob Johnson', 'Alice Williams', 'Charlie Brown');
const validPhoneArbitrary = fc.constantFrom('+11234567890', '+441234567890', '+911234567890', '555-123-4567');
const validLocationArbitrary = fc.constantFrom('New York, NY', 'San Francisco, CA', 'London, UK', 'Remote', 'Austin, TX');
...
describe('Property 9: Resume file validation', () => {
    ...
    it('should accept valid file formats (PDF, DOC, DOCX) under 5MB', () => {
        fc.assert(fc.property(filenameArbitrary, validExtensionArbitrary, validMimeTypeArbitrary, validFileSizeArbitrary, (filename, ext, mimetype, size) => {
            const file = {
    ...
    it('should reject invalid file extensions', () => {
        fc.assert(fc.property(filenameArbitrary, invalidExtensionArbitrary, validMimeTypeArbitrary, validFileSizeArbitrary, (filename, ext, mimetype, size) => {
            const file = {
                originalname: `${filename}${ext}`,
                mimetype,
                size,
    ...
    it('should reject invalid MIME types', () => {
        fc.assert(fc.property(filenameArbitrary, validExtensionArbitrary, invalidMimeTypeArbitrary, validFileSizeArbitrary, (filename, ext, mimetype, size) => {
            const file = {
                originalname: `${filename}${ext}`,
                mimetype,
                size,
    ...
    it('should reject files exceeding 5MB', () => {
        fc.assert(fc.property(filenameArbitrary, validExtensionArbitrary, validMimeTypeArbitrary, invalidFileSizeArbitrary, (filename, ext, mimetype, size) => {
            const file = {
                originalname: `${filename}${ext}`,
                mimetype,
                size,
...
describe('Property 10: Terms agreement required', () => {
    ...
    it('should require terms agreement for application submission', () => {
        fc.assert(fc.property(validNameArbitrary, validEmailArbitrary, validPhoneArbitrary, validLocationArbitrary, uuidArbitrary, (fullName, email, phone, location, jobId) => {
...
describe('Property 11: Application submission round-trip', () => {
    ...
    it('should create candidate and job association for valid application', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, uuidArbitrary, uuidArbitrary, validNameArbitrary, validEmailArbitrary, validPhoneArbitrary, validLocationArbitrary, async (companyId, jobId, candidateId, jobCandidateId, fullName, email, phone, location) => {
            const now = new Date();
...
describe('Property 12: Existing email updates candidate', () => {
    ...
    it('should update existing candidate when email already exists', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, uuidArbitrary, validNameArbitrary, validEmailArbitrary, validPhoneArbitrary, validLocationArbitrary, validNameArbitrary, validPhoneArbitrary, validLocationArbitrary, async (companyId, job2Id, candidateId, name1, email, phone1, location1, name2, phone2, location2) => {
            const now = new Date();
            const appliedStageId = 'applied-stage-id';
    ...
    it('should add new job application to existing candidate', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, uuidArbitrary, validEmailArbitrary, async (job1Id, job2Id, candidateId, _email) => {
...
describe('Property 13: Initial pipeline stage is Applied', () => {
    ...
    it('should set initial stage to Applied for new applications', () => {
        fc.assert(fc.property(uuidArbitrary, uuidArbitrary, uuidArbitrary, uuidArbitrary, (_companyId, jobId, candidateId, jobCandidateId) => {
            const now = new Date();
    ...
    it('should always use Applied stage regardless of other available stages', () => {
        fc.assert(fc.property(uuidArbitrary, uuidArbitrary, fc.array(fc.string({ minLength: 1, maxLength: 20 }), { minLength: 1, maxLength: 5 }), (jobId, candidateId, otherStageNames) => {
            const now = new Date();
...
//# sourceMappingURL=application.property.test.js.map