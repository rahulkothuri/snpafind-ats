...
function createMockDbJob(jobId, companyId, title, department, location, options = {}) {
    const now = new Date();
    return {
        id: jobId,
        companyId,
...
function createMockStages(jobId) {
    const now = new Date();
...
describe('Property 10: Job validation rejects missing required fields', () => {
    ...
    it('should reject job creation when multiple required fields are missing', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, fc.constantFrom('', '   '), fc.constantFrom('', '   '), async (companyId, emptyTitle, emptyDepartment) => {
            await expect(jobService.create({
                companyId,
                title: emptyTitle,
                department: emptyDepartment,
...
describe('Property 6: Job validation rejects missing required fields (Phase 2)', () => {
    ...
    it('should reject job with missing title and return validation error', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, fc.constantFrom('', '   ', '\t', '\n', undefined), departmentArbitrary, locationArbitrary, async (companyId, invalidTitle, department, location) => {
            try {
    ...
    it('should reject job with missing department and return validation error', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, jobTitleArbitrary, fc.constantFrom('', '   ', '\t', '\n', undefined), locationArbitrary, async (companyId, title, invalidDepartment, location) => {
            try {
                await jobService.create({
                    companyId,
                    title,
...
describe('Property 3: Multi-Select Locations Persistence', () => {
    ...
    it('should persist and retrieve the exact same locations after job creation', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, jobTitleArbitrary, departmentArbitrary, locationsArbitrary, async (jobId, companyId, title, department, selectedLocations) => {
    ...
    it('should persist empty locations array correctly', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, jobTitleArbitrary, departmentArbitrary, async (jobId, companyId, title, department) => {
    ...
    it('should preserve locations after job update', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, jobTitleArbitrary, departmentArbitrary, locationsArbitrary, locationsArbitrary, async (jobId, companyId, title, department, initialLocations, updatedLocations) => {
