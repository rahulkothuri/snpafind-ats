...
const locationArbitrary = fc.constantFrom('Bangalore', 'Hyderabad', 'Chennai', 'Pune', 'Remote');
const sourceArbitrary = fc.constantFrom('LinkedIn', 'Referral', 'Job Board', 'Career Page', 'Agency');
beforeEach(() => {
    vi.clearAllMocks();
});
describe('Property 19: Search returns matching candidates', () => {
    ...
    it('should return candidates matching query in name, email, or phone', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, fc.array(fc.tuple(uuidArbitrary, nameArbitrary, emailArbitrary, fc.option(phoneArbitrary, { nil: undefined }), locationArbitrary, sourceArbitrary), { minLength: 1, maxLength: 10 }), fc.constantFrom('John', 'jane', 'kumar', 'example.com', '+1234'), async (companyId, candidateDataList, searchQuery) => {
            const now = new Date();
            ...
            const expectedMatches = mockCandidates.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    it('should return empty array when no candidates match', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, fc.string({ minLength: 10, maxLength: 20 }).filter(s => !s.includes('@') && !s.includes('+')), async (companyId, nonMatchingQuery) => {
    ...
    it('should filter by location correctly', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, locationArbitrary, async (companyId, location) => {
            const now = new Date();
    ...
    it('should filter by experience range correctly', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, fc.integer({ min: 0, max: 10 }), fc.integer({ min: 11, max: 30 }), async (companyId, minExp, maxExp) => {
            const now = new Date();
    ...
    it('should filter by source correctly', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, sourceArbitrary, async (companyId, source) => {
            const now = new Date();
            const matchingCandidate = {
                id: 'candidate-1',
                companyId,
