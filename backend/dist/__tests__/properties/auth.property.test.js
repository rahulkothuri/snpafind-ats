...
describe('Property 1: Valid credentials produce valid JWT token', () => {
    it('should generate a valid JWT token for any valid user credentials', async () => {
        await fc.assert(fc.asyncProperty(userIdArbitrary, companyIdArbitrary, userRoleArbitrary, async (userId, companyId, role) => {
            // Create JWT payload
            const payload = {
                userId,
                companyId,
...
describe('Property 2: Invalid credentials produce generic error', () => {
    it('should verify that password hashing produces different hashes for same password', async () => {
        await fc.assert(fc.asyncProperty(validPasswordArbitrary, async (password) => {
    ...
    it('should verify that wrong passwords fail verification', async () => {
        await fc.assert(fc.asyncProperty(validPasswordArbitrary, validPasswordArbitrary.filter(p => p.length >= 8), async (correctPassword, wrongPassword) => {
    ...
    it('should verify tokens with wrong secret fail verification', async () => {
        await fc.assert(fc.asyncProperty(userIdArbitrary, companyIdArbitrary, userRoleArbitrary, fc.string({ minLength: 10, maxLength: 50 }), async (userId, companyId, role, wrongSecret) => {
            const payload = {
                userId,
                companyId,
                role,
...
describe('Authentication error message consistency', () => {
    it('should produce consistent error structure regardless of failure reason', () => {
        ...
        fc.assert(fc.property(fc.integer({ min: 1, max: 100 }), (_iteration) => {
