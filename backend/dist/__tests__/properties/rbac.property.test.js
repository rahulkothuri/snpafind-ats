...
import { rolePermissions, hasPermission, hasAllPermissions, hasAnyPermission, getPermissionsForRole, canAccessResource, isRoleHigherOrEqual, roleHierarchy, } from '../../middleware/rbac.js';
...
const resourceTypeArbitrary = fc.constantFrom('company', 'user', 'job', 'candidate', 'pipeline', 'interview', 'settings', 'reports');
...
describe('Property 5: Role-based access restriction', () => {
    it('should grant admin access to all features', async () => {
        await fc.assert(fc.asyncProperty(permissionArbitrary, async (permission) => {
    ...
    it('should restrict recruiter from admin-only features', async () => {
        ...
        await fc.assert(fc.asyncProperty(fc.constantFrom(...adminOnlyPermissions), async (permission) => {
    ...
    it('should grant recruiter access to candidate management features', async () => {
        ...
        await fc.assert(fc.asyncProperty(fc.constantFrom(...recruiterPermissions), async (permission) => {
    ...
    it('should grant hiring manager access to job requisitions and candidate review', async () => {
        ...
        await fc.assert(fc.asyncProperty(fc.constantFrom(...hiringManagerPermissions), async (permission) => {
    ...
    it('should deny access to non-existent permissions for all roles', async () => {
        ...
        await fc.assert(fc.asyncProperty(userRoleArbitrary, fc.constantFrom(...nonExistentPermissions), async (role, permission) => {
...
describe('Permission helper functions', () => {
    it('hasAllPermissions should return true only when all permissions are granted', async () => {
        await fc.assert(fc.asyncProperty(userRoleArbitrary, fc.array(permissionArbitrary, { minLength: 1, maxLength: 5 }), async (role, permissions) => {
    ...
    it('getPermissionsForRole should return all permissions for a role', async () => {
        await fc.assert(fc.asyncProperty(userRoleArbitrary, async (role) => {
    ...
    it('canAccessResource should correctly check resource:action permissions', async () => {
        await fc.assert(fc.asyncProperty(userRoleArbitrary, resourceTypeArbitrary, actionArbitrary, async (role, resourceType, action) => {
...
describe('Role hierarchy', () => {
    it('admin should be higher or equal to all roles', async () => {
        await fc.assert(fc.asyncProperty(userRoleArbitrary, async (targetRole) => {
    ...
    it('role hierarchy values should be consistent', async () => {
        await fc.assert(fc.asyncProperty(userRoleArbitrary, userRoleArbitrary, async (role1, role2) => {
...
describe('Permission consistency', () => {
    it('admin permissions should be a superset of all other roles', async () => {
        ...
        await fc.assert(fc.asyncProperty(fc.constantFrom('hiring_manager', 'recruiter'), async (role) => {
