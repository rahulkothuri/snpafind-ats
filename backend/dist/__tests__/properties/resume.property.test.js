...
describe('Property 17: Resume upload association', () => {
    ...
    it('should associate uploaded resume with candidate profile', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, filenameArbitrary, validExtensionArbitrary, async (candidateId, companyId, filename, ext) => {
            const now = new Date();
    ...
    it('should preserve other candidate fields when updating resume', async () => {
        await fc.assert(fc.asyncProperty(uuidArbitrary, uuidArbitrary, fc.string({ minLength: 1, maxLength: 50 }), fc.emailAddress(), fc.float({ min: 0, max: 30, noNaN: true }), validExtensionArbitrary, async (candidateId, companyId, name, email, experience, ext) => {
            const now = new Date();
            const resumeUrl = `/uploads/resumes/resume-${Date.now()}${ext}`;
            const existingCandidate = {
                id: candidateId,
...
describe('Property 18: Resume format validation', () => {
    ...
    it('should accept valid file formats (PDF, DOC, DOCX)', async () => {
        await fc.assert(fc.asyncProperty(filenameArbitrary, validExtensionArbitrary, validMimeTypeArbitrary, validFileSizeArbitrary, async (filename, ext, mimetype, size) => {
            const file = {
    ...
    it('should reject invalid file extensions', async () => {
        await fc.assert(fc.asyncProperty(filenameArbitrary, invalidExtensionArbitrary, validMimeTypeArbitrary, validFileSizeArbitrary, async (filename, ext, mimetype, size) => {
            const file = {
                originalname: `${filename}${ext}`,
                mimetype,
                size,
    ...
    it('should reject invalid MIME types', async () => {
        await fc.assert(fc.asyncProperty(filenameArbitrary, validExtensionArbitrary, invalidMimeTypeArbitrary, validFileSizeArbitrary, async (filename, ext, mimetype, size) => {
            const file = {
                originalname: `${filename}${ext}`,
                mimetype,
                size,
    ...
    it('should reject files exceeding 10MB', async () => {
        await fc.assert(fc.asyncProperty(filenameArbitrary, validExtensionArbitrary, validMimeTypeArbitrary, invalidFileSizeArbitrary, async (filename, ext, mimetype, size) => {
            const file = {
                originalname: `${filename}${ext}`,
                mimetype,
                size,
