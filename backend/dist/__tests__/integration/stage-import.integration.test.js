...
afterAll(async () => {
    ...
    try {
        ...
        // Delete users
        if (recruiterUserId)
            await prisma.user.delete({ where: { id: recruiterUserId } });
        if (adminUserId)
...
describe('Integration: Stage Import Functionality', () => {
    ...
    it('should control access to stage templates based on user role', async () => {
        ...
        const adminTemplates = await stageTemplateService.getAccessibleTemplates(adminUserId, 'admin', testCompanyId);
        ...
        const recruiterTemplates = await stageTemplateService.getAccessibleTemplates(recruiterUserId, 'recruiter', testCompanyId);
        const recruiterTemplate = recruiterTemplates.find(t => t.id === templateId);
        expect(recruiterTemplate).toBeUndefined();
        // Make template public
        await stageTemplateService.update(templateId, { isPublic: true }, adminUserId, 'admin');
        // Now recruiter should see the public template
        const recruiterPublicTemplates = await stageTemplateService.getAccessibleTemplates(recruiterUserId, 'recruiter', testCompanyId);
    ...
    it('should import stages from source job to target job', async () => {
        ...
        const importedTemplate = await stageTemplateService.importFromJob(sourceJobId, 'Imported Engineering Pipeline', 'Pipeline imported from source job', adminUserId, 'admin', testCompanyId);
    ...
    it('should allow stage modification during import process', async () => {
        ...
        const jobTemplates = await stageTemplateService.getJobStageTemplates(adminUserId, 'admin', testCompanyId);
    ...
    it('should handle errors during stage import process', async () => {
        ...
        await expect(stageTemplateService.importFromJob('non-existent-job-id', 'Invalid Import', 'Should fail', adminUserId, 'admin', testCompanyId)).rejects.toThrow();
        // Test import with duplicate template name
        await expect(stageTemplateService.create({
        ...
        await expect(stageTemplateService.getById(templateId, 'unauthorized-user-id', 'recruiter', 'different-company-id')).rejects.toThrow();
    }, 30000);
    /**
    ...
    it('should support full CRUD operations on stage templates', async () => {
        ...
        const retrievedTemplate = await stageTemplateService.getById(crudTemplate.id, adminUserId, 'admin', testCompanyId);
        ...
        const updatedTemplate = await stageTemplateService.update(crudTemplate.id, {
        ...
        // Verify deletion
        await expect(stageTemplateService.getById(crudTemplate.id, adminUserId, 'admin', testCompanyId)).rejects.toThrow();
    }, 30000);
    /**
    ...
    it('should ensure imported stages are independent of source job', async () => {
        ...
        const independenceTemplate = await stageTemplateService.importFromJob(sourceJobId, 'Independence Test Template', 'Testing stage independence', adminUserId, 'admin', testCompanyId);
        ...
        await stageTemplateService.update(independenceTemplate.id, { stages: modifiedStages }, adminUserId, 'admin');
        ...
        const updatedTemplate = await stageTemplateService.getById(independenceTemplate.id, adminUserId, 'admin', testCompanyId);
...
//# sourceMappingURL=stage-import.integration.test.js.map