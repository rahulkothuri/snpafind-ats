...
import { bulkImportService, parseCSV } from '../../services/bulkImport.service.js';
// Test data
let testCompanyId;
let testJobId;
let testUserId;
const testCandidateIds = [];
const testJobCandidateIds = [];
beforeAll(async () => {
    // Create test company
...
Invalid User,not-an-email`;
    ...
    describe('Bulk Import Service', () => {
        it('should import candidates and place them in Queue stage', async () => {
            ...
            const result = await bulkImportService.importCandidates(testJobId, testCompanyId, candidates, testUserId, false // Don't send emails in tests
            );
            ...
            const importedJCs = jobCandidates.filter(jc => candidates.some(c => c.email === jc.candidate.email));
        ...
        it('should skip candidates already in pipeline', async () => {
            ...
            const result = await bulkImportService.importCandidates(testJobId, testCompanyId, candidates, testUserId, false);
        ...
        it('should update existing candidate records', async () => {
            ...
            const result = await bulkImportService.importCandidates(testJobId, testCompanyId, [{ name: 'Updated Name', email, location: 'New Location' }], testUserId, false);
            expect(result.importedCount).toBe(1);
    ...
    describe('Queue to Applied Transition', () => {
        ...
        it('should have candidates in Queue stage after bulk import', async () => {
            ...
            const queueCandidates = jobCandidates.filter(jc => jc.currentStage.name === 'Queue');
            expect(queueCandidates.length).toBeGreaterThan(0);
        }, 30000);
    });
});
//# sourceMappingURL=bulk-import.integration.test.js.map