...
let testCompanyId;
let testUserId;
let testRecipientUserId; // User who receives notifications
let testJobId;
let testCandidateIds = [];
let testJobCandidateIds = [];
let appliedStageId;
let screeningStageId;
let interviewStageId;
beforeAll(async () => {
...
describe('Integration: 24.1 Test complete bulk move flow', () => {
    ...
    it('should create notifications for stage changes', async () => {
        ...
        const stageChangeNotifications = notifications.filter(n => n.type === 'stage_change');
...
describe('Integration: 24.2 Test candidate profile page', () => {
    ...
    it('should show all activities in timeline', async () => {
        ...
        const stageChangeActivities = activities.filter(a => a.activityType === 'stage_change');
        expect(stageChangeActivities.length).toBeGreaterThan(0);
        // Should include note added activities
        const noteActivities = activities.filter(a => a.activityType === 'note_added');
...
describe('Integration: 24.3 Test notifications flow', () => {
    ...
    it('should mark notification as read and update count', async () => {
        ...
        const { notifications: beforeNotifications, unreadCount: beforeCount } = await notificationService.getNotifications(testRecipientUserId);
        ...
        if (unreadNotification) {
            ...
            const updatedNotification = await notificationService.markAsRead(unreadNotification.id, testRecipientUserId);
    ...
    it('should dismiss SLA alert when candidate moves to new stage', async () => {
        ...
        if (breachCandidate) {
            ...
            if (jobCandidate) {
                ...
                const stillBreached = newBreaches.find(b => b.candidateId === breachCandidate.candidateId && b.stageName === 'Screening');
                expect(stillBreached).toBeUndefined();
            }
        }
    }, 30000);
});
//# sourceMappingURL=phase2-workflow.integration.test.js.map