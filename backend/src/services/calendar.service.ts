import { oauthService, OAuthTokenData, OAuthProvider } from './oauth.service.js';
...
export const calendarService = {
   ...
   */
  async refreshGoogleToken(refreshToken: string): Promise<OAuthTokenData> {
    if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
      throw new ValidationError({ google: ['Google OAuth is not configured'] });
    }
   ...
   */
  async refreshMicrosoftToken(refreshToken: string): Promise<OAuthTokenData> {
    if (!MICROSOFT_CLIENT_ID || !MICROSOFT_CLIENT_SECRET) {
      throw new ValidationError({ microsoft: ['Microsoft OAuth is not configured'] });
    }
   ...
   */
  async disconnectCalendar(userId: string, provider: OAuthProvider): Promise<void> {
    // Delete OAuth token
  ...
  async getGoogleAccessToken(userId: string): Promise<string> {
    const accessToken = await oauthService.getValidAccessToken(
      userId,
      'google',
      this.refreshGoogleToken.bind(this)
    );
  ...
  async updateGoogleCalendarEvent(
    ...
    const response = await fetch(
      `https://www.googleapis.com/calendar/v3/calendars/primary/events/${calendarEvent.externalId}?sendUpdates=all`,
      {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${accessToken}`,
    ...
    const updatedEvent = await response.json() as {
      id: string;
      conferenceData?: {
        entryPoints?: Array<{ uri: string; entryPointType: string }>;
      };
    ...
    const meetingLink = updatedEvent.conferenceData?.entryPoints?.find(
      ep => ep.entryPointType === 'video'
    )?.uri;
   ...
   * Requirements: 4.4
   */
  async deleteGoogleCalendarEvent(userId: string, interviewId: string): Promise<void> {
    const accessToken = await this.getGoogleAccessToken(userId);
  ...
  async getMicrosoftAccessToken(userId: string): Promise<string> {
    const accessToken = await oauthService.getValidAccessToken(
      userId,
      'microsoft',
      this.refreshMicrosoftToken.bind(this)
    );
  ...
  async updateMicrosoftCalendarEvent(
    ...
    const response = await fetch(
      `https://graph.microsoft.com/v1.0/me/events/${calendarEvent.externalId}`,
      {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${accessToken}`,
   ...
   * Requirements: 5.4
   */
  async deleteMicrosoftCalendarEvent(userId: string, interviewId: string): Promise<void> {
    const accessToken = await this.getMicrosoftAccessToken(userId);
   ...
   */
  async createCalendarEventsForInterview(
    interviewId: string,
    panelMemberIds: string[],
    input: Omit<CalendarEventInput, 'interviewId'>
   ...
   */
  async updateCalendarEventsForInterview(
    interviewId: string,
    panelMemberIds: string[],
    input: CalendarEventUpdateInput
  ): Promise<void> {
    for (const userId of panelMemberIds) {
      ...
      for (const event of calendarEvents) {
        try {
          ...
          console.error(
            `Failed to update ${event.provider} calendar event for user ${userId}:`,
            error
          );
        }
      }
   ...
   */
  async deleteCalendarEventsForInterview(
    interviewId: string,
    panelMemberIds: string[]
  ): Promise<void> {
    for (const userId of panelMemberIds) {
      ...
      for (const event of calendarEvents) {
        try {
          ...
          console.error(
            `Failed to delete ${event.provider} calendar event for user ${userId}:`,
            error
          );
        }
      }
  ...
  async generateMeetingLink(
    ...
    eventInput: Omit<CalendarEventInput, 'interviewId' | 'createMeetingLink'>
  ): Promise<string | undefined> {
    if (mode === 'in_person') {
