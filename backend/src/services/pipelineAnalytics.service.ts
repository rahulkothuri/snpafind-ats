...
// Type for stage history aggregation result
interface StageHistoryAggregation {
  stageId: string;
  _avg: {
...
export const pipelineAnalyticsService = {
   ...
   * Requirements: 2.4, 4.1
   */
  async getStageMetrics(jobId: string): Promise<PipelineAnalytics> {
    ...
    const avgDurations = await (prisma as any).stageHistory.groupBy({
      by: ['stageId'],
      where: {
        stageId: { in: stageIds },
    ...
    for (const agg of avgDurations as StageHistoryAggregation[]) {
    ...
    const slaThresholdMap = new Map<string, number>();
    for (const config of slaConfigs as SLAConfigResult[]) {
      slaThresholdMap.set(config.stageName.toLowerCase(), config.thresholdDays);
    }

    // Calculate SLA breach counts per stage
    const now = new Date();
    const slaBreachCounts = new Map<string, number>();

    // Get all job candidates with their current stage entry time
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const jobCandidates: any[] = await (prisma.jobCandidate as any).findMany({
      where: { jobId },
      include: {
        currentStage: {
          select: { name: true },
    ...
    // Build stage metrics
    const stageMetrics: StageMetric[] = stages.map((stage) => ({
      stageId: stage.id,
   ...
   * Requirements: 2.4
   */
  async calculateOverallTAT(jobId: string): Promise<number> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const totalDurations = await (prisma as any).stageHistory.groupBy({
      by: ['jobCandidateId'],
      where: {
