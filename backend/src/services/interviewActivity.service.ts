 ...
 */
function formatForDisplay(date: Date, timezone: string): { full: string } {
  return {
...
export const interviewActivityService = {
   ...
   * Requirements: 15.1
   */
  async recordInterviewScheduled(interview: Interview): Promise<CandidateActivity> {
    ...
    const formattedDateTime = formatForDisplay(
   ...
   * Requirements: 15.2
   */
  async recordInterviewRescheduled(
    interview: Interview,
    oldScheduledAt: Date,
    oldTimezone: string,
    oldDuration?: number,
    ...
    const newFormattedDateTime = formatForDisplay(
      interview.scheduledAt,
      interview.timezone
    );
   ...
   * Requirements: 15.3
   */
  async recordInterviewCancelled(
    interview: Interview,
    reason?: string
  ): Promise<CandidateActivity> {
    if (!interview.jobCandidate) {
