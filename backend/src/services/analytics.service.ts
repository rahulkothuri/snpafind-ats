...
export class AnalyticsService {
  ...
  async getConversionRates(companyId: string, userId: string, userRole: UserRole, filters: AnalyticsFilters = {}): Promise<ConversionData> {
    ...
    const conversionStages: ConversionData['stages'] = [];
  ...
  async getRecruiterProductivity(companyId: string, userId: string, userRole: UserRole, filters: AnalyticsFilters = {}): Promise<RecruiterData[]> {
    ...
    for (const recruiter of recruiters) {
      ...
      const offersMade = allCandidates.filter(candidate => 
        candidate.currentStage.name === 'Offer'
      ).length;

      // Calculate hires (candidates in "Hired" stage)
      const hires = allCandidates.filter(candidate => 
        candidate.currentStage.name === 'Hired'
      ).length;

      // Calculate average time-to-fill for this recruiter's hires
      const hiredCandidates = allCandidates.filter(candidate => 
        candidate.currentStage.name === 'Hired'
      );
      ...
      if (hiredCandidates.length > 0) {
        const totalTimeToFill = hiredCandidates.reduce((sum, candidate) => {
          ...
          const job = recruiter.assignedJobs.find(job => 
            job.jobCandidates.some(jc => jc.id === candidate.id)
          );
          if (job) {
  ...
  async getPanelPerformance(companyId: string, userId: string, userRole: UserRole, filters: AnalyticsFilters = {}): Promise<PanelData[]> {
    ...
    const panelMemberStats = new Map<string, {
      name: string;
      interviewRounds: number;
      offerCount: number;
