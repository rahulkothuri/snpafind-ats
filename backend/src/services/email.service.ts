 ...
 */
const DEFAULT_TEMPLATES: Record<EmailTemplateKey, { subject: string; htmlContent: string }> = {
  interview_confirmation_candidate: {
...
export const emailService = {
   ...
   */
  async getCompanyTemplates(companyId: string): Promise<Array<{
    templateKey: EmailTemplateKey;
    subject: string;
    htmlContent: string;
    isCustom: boolean;
   ...
   * @returns Send result
   */
  async sendInterviewConfirmationToCandidate(interview: Interview): Promise<EmailSendResult> {
    const candidate = interview.jobCandidate?.candidate;
    if (!candidate?.email) {
   ...
   */
  async sendInterviewInvitationToPanel(interview: Interview): Promise<EmailSendResult[]> {
    const panelMembers = interview.panelMembers || [];
    const companyId = interview.jobCandidate?.job?.companyId;
   ...
   */
  async sendInterviewConfirmation(interview: Interview): Promise<{
   ...
   * @returns Send result
   */
  async sendRescheduleToCandidate(
    interview: Interview,
    oldScheduledAt: Date
   ...
   * @returns Send result
   */
  async sendCancellationToCandidate(interview: Interview): Promise<EmailSendResult> {
    const candidate = interview.jobCandidate?.candidate;
    if (!candidate?.email) {
      return { success: false, error: 'Candidate email not found' };
    }
   ...
   * @returns Array of send results
   */
  async sendCancellationToPanel(interview: Interview): Promise<EmailSendResult[]> {
    const panelMembers = interview.panelMembers || [];
    const companyId = interview.jobCandidate?.job?.companyId;
   ...
   * @returns Combined results
   */
  async sendCancellationNotification(interview: Interview): Promise<{
    candidateResult: EmailSendResult;
    panelResults: EmailSendResult[];
  }> {
    const [candidateResult, panelResults] = await Promise.all([
   ...
   * @returns Send result
   */
  async sendReminderToCandidate(interview: Interview): Promise<EmailSendResult> {
    const candidate = interview.jobCandidate?.candidate;
    if (!candidate?.email) {
      return { success: false, error: 'Candidate email not found' };
    }
   ...
   * @returns Send result
   */
  async sendReminderToPanelMember(
    interview: Interview,
    panelMember: User
  ): Promise<EmailSendResult> {
   ...
   */
  async getInterviewsNeedingCandidateReminder(): Promise<Interview[]> {
    const now = new Date();
    ...
    return interviews.map(i => ({
      ...
      jobCandidate: i.jobCandidate ? {
        ...
        candidate: i.jobCandidate.candidate ? {
          ...
          skills: Array.isArray(i.jobCandidate.candidate.skills) ? i.jobCandidate.candidate.skills as string[] : [],
        ...
        job: i.jobCandidate.job ? {
          ...
          status: i.jobCandidate.job.status as 'active' | 'paused' | 'closed',
          openings: i.jobCandidate.job.openings,
          skills: Array.isArray(i.jobCandidate.job.skills) ? i.jobCandidate.job.skills as string[] : [],
          locations: Array.isArray(i.jobCandidate.job.locations) ? i.jobCandidate.job.locations as string[] : [],
   ...
   */
  async getInterviewsNeedingPanelReminder(): Promise<Interview[]> {
    const now = new Date();
