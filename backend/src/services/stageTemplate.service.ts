...
// Stage template interface for storage and retrieval
...
export const stageTemplateService = {
   ...
   * Requirements: 3.1, 3.2
   */
  async getAccessibleTemplates(userId: string, userRole: UserRole, companyId: string): Promise<StageTemplate[]> {
   ...
   * Requirements: 3.1, 3.2
   */
  async getById(id: string, userId: string, userRole: UserRole, companyId: string): Promise<StageTemplate> {
   ...
   * Requirements: 3.2
   */
  async update(id: string, data: UpdateStageTemplateData, userId: string, userRole: UserRole): Promise<StageTemplate> {
   ...
   */
  async getJobStageTemplates(userId: string, userRole: UserRole, companyId: string): Promise<Array<{
    jobId: string;
    jobTitle: string;
    stages: PipelineStageConfig[];
    createdAt: Date;
  ...
  async importFromJob(
    ...
    const stages: PipelineStageConfig[] = job.pipelineStages.map(stage => ({
      name: stage.name,
      position: stage.position,
      isMandatory: stage.isMandatory,
      subStages: stage.subStages.map(sub => ({
  ...
  mapToStageTemplate(template: {
    ...
    name: string;
    description: string;
    stages: any; // JsonValue from Prisma
    isPublic: boolean;
    companyId: string;
    createdBy: string;
    createdAt: Date;
