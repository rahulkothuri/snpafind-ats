...
export interface BulkImportResult {
    ...
    skippedCount: number;  // Candidates already in pipeline
    failedCount: number;
 ...
 */
export function parseCSV(content: string): BulkImportCandidate[] {
 ...
 */
export async function parseExcel(buffer: Buffer | ArrayBuffer): Promise<BulkImportCandidate[]> {
    ...
    const rows = worksheet.getSheetValues() as (string | number | null | undefined)[][];
        ...
        headers.forEach((header, index) => {
            ...
            if (value !== null && value !== undefined) {
                if (typeof value === 'object' && value !== null) {
                    ...
                    cellValue = (value as any).text || (value as any).result || String(value);
                } else {
...
export const bulkImportService = {
    ...
    async importSingleCandidate(params: {
        ...
        companyName: string;
        sendEmail: boolean;
    }): Promise<{ imported: boolean; emailSent: boolean }> {
        const {
            candidateData, jobId, companyId, queueStageId, queueStageName,
            userId, jobTitle, companyName, sendEmail
        } = params;
        ...
        if (candidate) {
            ...
            candidate = await prisma.candidate.update({
                ...
                data: {
                    ...
                    skills: candidateData.skills as string[] || (candidate.skills as string[]),
                },
            });
        } else {
