...
export const oauthService = {
  ...
  async getValidAccessToken(
    userId: string,
    provider: OAuthProvider,
    refreshCallback?: (refreshToken: string) => Promise<OAuthTokenData>
  ): Promise<string | null> {
   ...
   */
  async hasValidToken(userId: string, provider: OAuthProvider): Promise<boolean> {
    const token = await this.getToken(userId, provider);
  ...
  async getConnectedProviders(userId: string): Promise<OAuthProvider[]> {
    ...
    return tokens.map(t => t.provider as OAuthProvider);
  },
