...
export const candidateService = {
  ...
  async searchWithScoring(companyId: string, filters: CandidateSearchFilters): Promise<Candidate[]> {
    ...
    let result = candidates.map((c: PrismaCandidateResult) => mapPrismaCandidateToCandidate(c));
  ...
  async addTag(candidateId: string, tag: string): Promise<Candidate> {
    ...
    if (tagExists) {
      ...
      return mapPrismaCandidateToCandidate(existing as PrismaCandidateResult);
    }
   ...
   */
  async addToJob(candidateId: string, jobId: string, companyId: string): Promise<{ message: string; jobCandidate: JobCandidate }> {
