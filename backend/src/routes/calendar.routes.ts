 ...
 */
router.get('/google/auth', authenticate, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
...
router.get('/google/callback', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (error) {
      ...
      return res.redirect(`${frontendUrl}/settings?calendar_error=${encodeURIComponent(error as string)}`);
    }
    ...
    await calendarService.handleGoogleCallback(userId, code as string);
 ...
 */
router.get('/microsoft/auth', authenticate, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
    const redirectUri = req.query.redirectUri as string | undefined;
...
router.get('/microsoft/callback', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (error) {
      ...
      return res.redirect(`${frontendUrl}/settings?calendar_error=${encodeURIComponent(errorMsg as string)}`);
    }
    ...
    await calendarService.handleMicrosoftCallback(userId, code as string);
 ...
 */
router.delete('/:provider/disconnect', authenticate, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
 ...
 */
router.get('/connected-providers', authenticate, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.user!.userId;
