...
import { emailService, EmailTemplateKey } from '../services/email.service.js';
 ...
 * Requirements: 16.4
 */
router.get('/:key', authenticate, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const companyId = req.user!.companyId;
    const templateKey = req.params.key as EmailTemplateKey;
 ...
 * Requirements: 16.4
 */
router.put('/:key', authenticate, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const companyId = req.user!.companyId;
    const templateKey = req.params.key as EmailTemplateKey;
 ...
 * Requirements: 16.4
 */
router.delete('/:key', authenticate, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const companyId = req.user!.companyId;
    const templateKey = req.params.key as EmailTemplateKey;
 ...
 * Requirements: 16.3
 */
router.post('/:key/preview', authenticate, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const templateKey = req.params.key as EmailTemplateKey;
