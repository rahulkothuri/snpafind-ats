...
router.get('/alerts', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const type = req.query.type as 'sla' | 'feedback' | 'all' | undefined;
 ...
 * Requirements: 10.5
 */
router.get('/settings/sla', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const companyId = authReq.user!.companyId;
 ...
 * Requirements: 10.5
 */
router.put('/settings/sla', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const companyId = authReq.user!.companyId;
 ...
 * Requirements: 10.5
 */
router.delete('/settings/sla/:stageName', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const companyId = authReq.user!.companyId;
 ...
 * Requirements: 10.5
 */
router.post('/settings/sla/apply-defaults', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const companyId = authReq.user!.companyId;
 ...
 */
router.get('/candidates/:id/sla-status', async (req: Request, res: Response, next: NextFunction) => {
  try {
