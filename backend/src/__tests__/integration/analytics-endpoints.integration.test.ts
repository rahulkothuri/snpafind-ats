...
beforeAll(async () => {
  ...
  await (prisma as any).sLAConfig.create({
    data: {
      companyId: testCompanyId,
      stageName: 'Applied',
      thresholdDays: 5,
...
describe('Integration: Analytics API Endpoints', () => {
  ...
  it('should return rejection reasons with correct structure', async () => {
    const result = await analyticsService.getRejectionReasons(
      testCompanyId,
      testUserId,
      'admin'
    );
  ...
  it('should return time-in-stage data with correct structure', async () => {
    const result = await analyticsService.getTimeInStage(
      testCompanyId,
      testUserId,
      'admin'
    );
  ...
  it('should return drop-off data with correct structure', async () => {
    const result = await analyticsService.getDropOffAnalysis(
      testCompanyId,
      testUserId,
      'admin'
    );
  ...
  it('should apply role-based filtering for recruiters', async () => {
    const result = await analyticsService.getKPIMetrics(
      testCompanyId,
      testUserId,
      'recruiter' // Recruiter role should only see their assigned jobs
    );
