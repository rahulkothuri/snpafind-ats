...
let recruiter1UserId: string;
let recruiter2UserId: string;
...
afterAll(async () => {
  ...
  try {
    ...
    // Delete users
    if (recruiter2UserId) await prisma.user.delete({ where: { id: recruiter2UserId } });
    if (recruiter1UserId) await prisma.user.delete({ where: { id: recruiter1UserId } });
    if (hiringManagerUserId) await prisma.user.delete({ where: { id: hiringManagerUserId } });
...
describe('Integration: Job Access Control Workflow', () => {
  ...
  it('should allow admin to access all jobs', async () => {
    const accessibleJobs = await jobAccessControlService.getAccessibleJobs(
      adminUserId,
      'admin',
      testCompanyId
    );
    ...
    const hasAccessToPublic = await jobAccessControlService.validateJobAccess(
      publicJobId,
      adminUserId,
      'admin'
    );
    ...
    const hasAccessToAssigned = await jobAccessControlService.validateJobAccess(
      assignedJobId,
      adminUserId,
      'admin'
    );
  ...
  it('should allow recruiter to access only assigned jobs', async () => {
    ...
    const recruiter1Jobs = await jobAccessControlService.getAccessibleJobs(
      recruiter1UserId,
      'recruiter',
      testCompanyId
    );
    ...
    const recruiter2Jobs = await jobAccessControlService.getAccessibleJobs(
      recruiter2UserId,
      'recruiter',
      testCompanyId
    );
    ...
    const recruiter2HasAccess = await jobAccessControlService.validateJobAccess(
      assignedJobId,
      recruiter2UserId,
      'recruiter'
    );
  ...
  it('should update permissions immediately when job assignment changes', async () => {
    ...
    let hasAccess = await jobAccessControlService.validateJobAccess(
      assignedJobId,
      recruiter2UserId,
      'recruiter'
    );
    ...
    hasAccess = await jobAccessControlService.validateJobAccess(
      assignedJobId,
      recruiter2UserId,
      'recruiter'
    );
    ...
    const recruiter1HasAccess = await jobAccessControlService.validateJobAccess(
      assignedJobId,
      recruiter1UserId,
      'recruiter'
    );
  ...
  it('should filter jobs correctly based on user role', async () => {
    ...
    const adminFiltered = jobAccessControlService.filterJobsByUserRole(
      allJobs,
      adminUserId,
      'admin'
    );
    ...
    const hmFiltered = jobAccessControlService.filterJobsByUserRole(
      allJobs,
      hiringManagerUserId,
      'hiring_manager'
    );
    ...
    const recruiter1Filtered = jobAccessControlService.filterJobsByUserRole(
      allJobs,
      recruiter1UserId,
      'recruiter'
    );
    ...
    const recruiter2Filtered = jobAccessControlService.filterJobsByUserRole(
      allJobs,
      recruiter2UserId,
      'recruiter'
    );
  ...
  it('should deny access to unauthorized users', async () => {
    ...
    const unauthorizedAccess = await jobAccessControlService.validateJobAccess(
      publicJobId,
      recruiter1UserId,
      'recruiter'
    );
    ...
    const nonExistentAccess = await jobAccessControlService.validateJobAccess(
      'non-existent-job-id',
      adminUserId,
      'admin'
    );
    ...
    const invalidRoleAccess = await jobAccessControlService.validateJobAccess(
      publicJobId,
      adminUserId,
      'invalid_role' as any
    );
  ...
  it('should prevent access to jobs from different companies', async () => {
    ...
    const otherUserJobs = await jobAccessControlService.getAccessibleJobs(
      otherUser.id,
      'admin',
      otherCompany.id
    );
    ...
    const hasAccessToOurJob = await jobAccessControlService.validateJobAccess(
      publicJobId,
      otherUser.id,
      'admin'
    );
