...
// Entity types that have complex JSON fields
...
function serializeCandidate(candidate: CandidateData): string {
...
function deserializeCandidate(json: string): CandidateData {
...
function serializeActivity(activity: ActivityData): string {
  return JSON.stringify({
    ...activity,
...
function deserializeActivity(json: string): ActivityData {
  const parsed = JSON.parse(json);
  return {
    ...parsed,
...
describe('Property 20: JSON serialization round-trip', () => {
  it('should preserve candidate data through serialization/deserialization', () => {
    fc.assert(
      fc.property(candidateArbitrary, (candidate) => {
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          email: fc.emailAddress(),
          skills: fc.array(
            fc.constantFrom('C++', 'C#', 'Node.js', 'React/Redux', 'AWS (Lambda)'),
