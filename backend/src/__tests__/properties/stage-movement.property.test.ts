...
const mockPrismaJobCandidate = (prisma as any).jobCandidate as {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (jobCandidateId, candidateId, jobId, fromStageIndex, toStageIndex) => {
          ...
          let updatedJobCandidate: any = null;
          ...
          expect((result.activity.metadata as any).fromStageId).toBe(fromStageId);
          expect((result.activity.metadata as any).toStageId).toBe(toStageId);
          expect((result.activity.metadata as any).fromStageName).toBe(fromStageName);
          expect((result.activity.metadata as any).toStageName).toBe(toStageName);
  ...
  it('should require rejection reason when moving to Rejected stage', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        uuidArbitrary,
        stageIndexArbitrary,
        async (jobCandidateId, candidateId, jobId, fromStageIndex) => {
          const now = new Date();
          const fromStageId = `stage-${jobId}-${fromStageIndex}`;
  ...
  it('should accept rejection reason and include it in activity when moving to Rejected', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (jobCandidateId, candidateId, jobId, fromStageIndex, rejectionReason) => {
          const now = new Date();
          const fromStageId = `stage-${jobId}-${fromStageIndex}`;
          const rejectedStageIndex = DEFAULT_STAGES.indexOf('Rejected');
          const rejectedStageId = `stage-${jobId}-${rejectedStageIndex}`;
          ...
          expect((result.activity.metadata as any).rejectionReason).toBe(rejectionReason);
        }
      ),
      { numRuns: 50 }
    );
  ...
  it('should fail when job candidate does not exist', async () => {
    await fc.assert(
      fc.asyncProperty(
        uuidArbitrary,
        uuidArbitrary,
        async (jobCandidateId, newStageId) => {
  ...
  it('should fail when new stage does not belong to the job pipeline', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        uuidArbitrary,
        uuidArbitrary,
        uuidArbitrary, // Invalid stage ID
        stageIndexArbitrary,
        async (jobCandidateId, candidateId, jobId, invalidStageId, fromStageIndex) => {
          const now = new Date();
          const fromStageId = `stage-${jobId}-${fromStageIndex}`;
