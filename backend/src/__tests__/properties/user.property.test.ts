  ...
  it('should return equivalent user after create and retrieve (password hashed)', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        userRoleArbitrary,
        async (userId, companyId, name, email, password, role) => {
          const now = new Date();
          ...
          expect((retrieved as unknown as { password?: string }).password).toBeUndefined();
          expect((retrieved as unknown as { passwordHash?: string }).passwordHash).toBeUndefined();
        }
      ),
      { numRuns: 50 }
    );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (userId, companyId, name, email, password, role) => {
          ...
          mockPrismaUser.create.mockImplementationOnce(async (args: { data: { passwordHash: string } }) => {
  ...
  it('should correctly map Prisma user to User type', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        userRoleArbitrary,
        fc.boolean(),
        async (userId, companyId, name, email, role, isActive) => {
          const now = new Date();
