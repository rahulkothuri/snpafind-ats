...
const mockPrismaCompany = prisma.company as unknown as {
...
const companyNameArbitrary = fc.constantFrom(
  'Acme Corp', 'TechStart Inc', 'Global Solutions', 'Innovation Labs', 'Digital Ventures'
);

const validEmailArbitrary = fc.constantFrom(
  'admin@example.com', 'contact@test.org', 'info@company.io', 'hr@acme.com', 'support@tech.net'
);

const validUrlArbitrary = fc.constantFrom(
  'https://example.com/logo.png', 'https://test.org/image.jpg', 'https://company.io/avatar.svg'
);

const addressArbitrary = fc.constantFrom(
  '123 Main St', '456 Oak Ave', '789 Tech Blvd', '100 Innovation Way', '200 Business Park'
);

const uuidArbitrary = fc.uuid();

// Enhanced company profile arbitraries for Phase 2
const websiteUrlArbitrary = fc.constantFrom(
  'https://acme.com', 'https://techstart.org', 'https://global.io', 'https://innovation.com'
);

const companySizeArbitrary = fc.constantFrom(
  '1-10 employees',
  '11-50 employees',
  '51-200 employees',
  '201-500 employees',
...
const descriptionArbitrary = fc.constantFrom(
  'A leading technology company', 'Innovative solutions provider', 'Global enterprise services'
);
...
const socialUrlArbitrary = fc.constantFrom(
  'https://linkedin.com/company/acme', 'https://twitter.com/acme', 'https://facebook.com/acme'
);
...
function createMockDbCompany(overrides: Partial<{
  id: string;
  name: string;
  contactEmail: string;
  logoUrl: string | null;
  ...
  it('should return equivalent company after create and retrieve', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.option(addressArbitrary, { nil: undefined }),
        async (id, name, contactEmail, logoUrl, address) => {
          const now = new Date();
  ...
  it('should persist updates correctly', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        companyNameArbitrary,
        validEmailArbitrary,
        async (id, originalName, originalEmail, updatedName, updatedEmail) => {
          const now = new Date();
...
describe('Property 4: Company IDs are unique', () => {
  it('should generate unique IDs for all created companies (UUID format)', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(uuidArbitrary, { minLength: 2, maxLength: 20 }),
        async (uuids) => {
  ...
  it('should verify UUID generation produces unique values', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.array(
          fc.tuple(companyNameArbitrary, validEmailArbitrary, uuidArbitrary),
  ...
  it('should return equivalent company with all enhanced fields after create and retrieve', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.option(validUrlArbitrary, { nil: undefined }),
        fc.option(addressArbitrary, { nil: undefined }),
        fc.option(websiteUrlArbitrary, { nil: undefined }),
        fc.option(companySizeArbitrary, { nil: undefined }),
        fc.option(industryArbitrary, { nil: undefined }),
        fc.option(descriptionArbitrary, { nil: undefined }),
        async (id, name, contactEmail, logoUrl, address, website, companySize, industry, description) => {
          const now = new Date();
  ...
  it('should persist all enhanced profile fields on update', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.option(cityArbitrary, { nil: undefined }),
        fc.option(stateArbitrary, { nil: undefined }),
        fc.option(countryArbitrary, { nil: undefined }),
        fc.option(postalCodeArbitrary, { nil: undefined }),
        fc.option(socialUrlArbitrary, { nil: undefined }),
        fc.option(brandColorArbitrary, { nil: undefined }),
        async (id, name, contactEmail, phone, city, state, country, postalCode, linkedinUrl, brandColor) => {
          const now = new Date();
          const later = new Date(now.getTime() + 1000);
