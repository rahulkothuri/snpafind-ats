...
const stageNameArbitrary = fc.constantFrom(
  'Phone Screen', 'Technical Interview', 'HR Round', 
  'Manager Interview', 'Final Round', 'Background Check'
);
  ...
  it('should insert custom stage at specified position and shift subsequent stages', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        uuidArbitrary,
        stageNameArbitrary,
        fc.integer({ min: 0, max: 7 }), // Position within default stages
...
describe('Property 13: Stage reordering maintains candidate associations', () => {
  ...
  it('should update positions correctly when moving stage down', async () => {
    await fc.assert(
      fc.asyncProperty(
        uuidArbitrary,
        fc.integer({ min: 0, max: 5 }), // Old position
        ...
        async (jobId, oldPosition, newPosition) => {
          ...
          if (actualNewPosition >= DEFAULT_STAGES.length) return; // Skip invalid positions
  ...
  it('should update positions correctly when moving stage up', async () => {
    await fc.assert(
      fc.asyncProperty(
        uuidArbitrary,
        fc.integer({ min: 2, max: 7 }), // Old position (must be > 0 to move up)
        ...
        async (jobId, oldPosition, newPosition) => {
          ...
          if (actualNewPosition < 0) return; // Skip invalid positions
