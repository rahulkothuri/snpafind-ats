...
const mockPrismaCandidate = (prisma as any).candidate as {
  ...
  it('should persist score and return updated candidate for any valid score', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        emailArbitrary,
        scoreArbitrary,
        fc.option(scoreArbitrary, { nil: undefined }), // Old score (optional)
        async (candidateId, companyId, name, email, newScore, oldScore) => {
          ...
          expect((result.activity.metadata as any).oldScore).toBe(oldScore);
          expect((result.activity.metadata as any).newScore).toBe(newScore);
        }
      ),
      { numRuns: 100 }
    );
  ...
  it('should reject scores outside valid range (0-100)', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (candidateId, invalidScore) => {
          // Should reject without even checking the database
          await expect(
            candidateService.updateScore(candidateId, invalidScore)
          ).rejects.toThrow();
        }
      ),
      { numRuns: 50 }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (candidateId, score) => {
          ...
          await expect(
            candidateService.updateScore(candidateId, score)
          ).rejects.toThrow();
        }
      ),
      { numRuns: 20 }
  ...
  it('should handle score updates from unset to set', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        emailArbitrary,
        scoreArbitrary,
        async (candidateId, companyId, name, email, newScore) => {
          const now = new Date();
          ...
          expect((result.activity.metadata as any).oldScore).toBeNull();
          expect((result.activity.metadata as any).newScore).toBe(newScore);
        }
      ),
      { numRuns: 50 }
  ...
  it('should correctly categorize scores by color coding ranges', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        uuidArbitrary,
        uuidArbitrary,
        fc.oneof(highScoreArbitrary, mediumScoreArbitrary, lowScoreArbitrary),
        async (candidateId, companyId, score) => {
          const now = new Date();
