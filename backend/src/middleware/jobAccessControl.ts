...
export function requireJobAccess() {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const hasAccess = await jobAccessControlService.validateJobAccess(
        jobId,
        req.user.userId,
        req.user.role
      );
  ...
  return (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    res.json = function(data: any) {
      ...
      if (Array.isArray(data)) {
        ...
        const filteredData = jobAccessControlService.filterJobsByUserRole(
          data,
          req.user!.userId,
          req.user!.role
        );
        ...
        jobAccessControlService.validateJobAccess(
          data.id,
          req.user!.userId,
          req.user!.role
        ).then(hasAccess => {
